...
func TestParallelRoundTrip(t *testing.T) {
	...
	err := steg.EncodeParallel(m, pass, bytes.NewReader(payload))
	require.NoError(t, err)

	got, err := steg.DecodeParallel(m, pass)
	require.NoError(t, err)
...
func BenchmarkEncodeSequential(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		m := newTestImage()
		if err := steg.Encode(m, pass, bytes.NewReader(payload)); err != nil {
			b.Fatal(err)
		}
	}
}
...
func BenchmarkEncodeParallel(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		m := newTestImage()
		if err := steg.EncodeParallel(m, pass, bytes.NewReader(payload)); err != nil {
			b.Fatal(err)
		}
	}
}
...
func BenchmarkDecodeSequential(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		if _, err := steg.Decode(m, pass); err != nil {
			b.Fatal(err)
		}
	}
}
...
func BenchmarkDecodeParallel(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		if _, err := steg.DecodeParallel(m, pass); err != nil {
			b.Fatal(err)
		}
	}
}
...
func BenchmarkDecodeBySize(b *testing.B) {
	...
	for _, tc := range sizeCases {
		...
		if err := steg.Encode(mSeq, pass, bytes.NewReader(payload)); err != nil {
			b.Fatal(err)
		}
		mPar := image.NewRGBA(image.Rect(0, 0, tc.w, tc.h))
		if err := steg.EncodeParallel(mPar, pass, bytes.NewReader(payload)); err != nil {
			b.Fatal(err)
		}

		b.Run(tc.name+"/seq", func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				if _, err := steg.Decode(mSeq, pass); err != nil {
					b.Fatal(err)
				}
			}
		})
		b.Run(tc.name+"/par", func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				if _, err := steg.DecodeParallel(mPar, pass); err != nil {
					b.Fatal(err)
				}
			}
		})
